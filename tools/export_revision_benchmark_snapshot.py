...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the current revision benchmark summary as JSON or text.",
    )
    parser.add_argument(
        "--format",
        choices=("json", "text"),
...
def main() -> int:
    ...
    summary = build_revision_benchmark_summary(metadata=metadata)
    if args.format == "text":
        payload = format_revision_benchmark_summary(summary)
    else:
